...
{id:"blend",name:"Фраза-квест",short:"Phrase",mins:7},
{id:"practice",name:"Игра",short:"Play",mins:6,game:true},
{id:"read",name:"Слова + история",short:"Story",mins:6,game:true},
{id:"exit",name:"Секретная фраза",short:"Final game",mins:3,game:true}
];
...
function review(l){
 const vocab=l.vocab||[];
 const body=`<div class="kid-task"><div class="task-mascot">🃏</div><h4>Word Cards</h4><p>Прочитай слово и сразу проверь смысл.</p><div class="meaning-grid">${vocab.map(x=>`<div class="meaning-card open"><b>${esc(x.w)}</b><span class="meaning-reveal"><span class="meaning-pic">${x.icon}</span><small>${esc(x.ru||"")}</small></span></div>`).join("")}</div></div>`;
 return frame("Разминка: слова со смыслом","Слово + картинка + перевод.",body,"Попросите ребёнка сначала прочитать английское слово, затем назвать значение. Все дополнительные слова тоже даны со значением.",l.extra);
}
function wireReview(){}
function notice(l){
...
function speakSound(g,cue){
 if(!("speechSynthesis" in window)){alert("На этом устройстве голос недоступен.");return}
 const examples={s:"snake",a:"cat",t:"top",p:"pen",i:"sit",n:"net",m:"map",e:"pen",o:"dog",u:"sun",sh:"ship",ch:"chip",th:"thin",ng:"sing",ck:"duck",st:"stop",sp:"spin",bl:"black",cl:"clap",fl:"flat","w / v":"win van","a_e":"cape","i_e":"kite","o_e":"home","u_e":"cube",ee:"see",ea:"eat",ai:"rain",ay:"day",oa:"boat",ow:"snow",oo:"moon",the:"the",said:"said",one:"one",some:"some",are:"are",was:"was"};
 speechSynthesis.cancel();let text=examples[g]||g.replace("_","e");let u=new SpeechSynthesisUtterance(text);u.lang="en-GB";u.rate=.58;u.pitch=1;u.volume=1;speechSynthesis.speak(u);
}
...
function read(l){
 const q=l.comprehension,story=l.story.map((x,i)=>{const t=typeof x==="string"?x:x.t,ru=typeof x==="string"?"":x.ru;return `<div class="story-line"><p>${highlight(t,l.patterns)}</p>${ru?`<button class="story-translate" data-story-ru="${i}" type="button">🇷🇺 перевод</button><small class="story-ru" hidden>${esc(ru)}</small>`:""}</div>`}).join(""),vocab=l.vocab||[];
...
function wireRead(l){
 const q=l.comprehension;document.querySelectorAll("[data-story-ru]").forEach(b=>b.onclick=()=>{const s=b.nextElementSibling;s.hidden=!s.hidden;b.textContent=s.hidden?"🇷🇺 перевод":"Спрятать перевод"});document.querySelectorAll("[data-read-answer]").forEach(b=>b.onclick=()=>{document.querySelectorAll("[data-read-answer]").forEach(x=>x.classList.remove("correct","wrong"));let ok=b.dataset.readAnswer===q.a;b.classList.add(ok?"correct":"wrong");document.getElementById("readFeedback").textContent=ok?"Yes! Find the proof in the story.":"Try again. Read the story clue."});
 let t=document.getElementById("toggleMarks");if(l.patterns.length){let on=true;t.onclick=()=>{on=!on;document.getElementById("storyText");document.querySelector(".story").innerHTML=`<div class="mini-label">MINI-STORY</div>`+(on?l.story.map((x,i)=>{let t=typeof x==="string"?x:x.t,ru=typeof x==="string"?"":x.ru;return `<div class="story-line"><p>${highlight(t,l.patterns)}</p>${ru?`<button class="story-translate" data-story-ru="${i}" type="button">🇷🇺 перевод</button><small class="story-ru" hidden>${esc(ru)}</small>`:""}</div>`}):l.story.map((x,i)=>{let t=typeof x==="string"?x:x.t,ru=typeof x==="string"?"":x.ru;return `<div class="story-line"><p>${esc(t)}</p>${ru?`<button class="story-translate" data-story-ru="${i}" type="button">🇷🇺 перевод</button><small class="story-ru" hidden>${esc(ru)}</small>`:""}</div>`}).join("");document.querySelectorAll("[data-story-ru]").forEach(b=>b.onclick=()=>{const s=b.nextElementSibling;s.hidden=!s.hidden;b.textContent=s.hidden?"🇷🇺 перевод":"Спрятать перевод"});t.textContent=on?"Hide code":"Show code"}}else t.disabled=true;
...
function exit(l){
 const secret=l.secret||(typeof l.story[0]==="string"?l.story[0]:l.story[0].t);
 const words=secret.replace(/[.!?]/g,"").split(/\s+/);const body=`<div class="final-vault"><div class="vault-top"><span>🔐 FINAL BOSS</span><b>SECRET PHRASE VAULT</b><small>Увидел → запомнил → собрал → открыл сейф</small></div><div class="vault-door" id="flashDisplay">🔒 CLASSIFIED</div><div class="vault-timer" id="vaultTimer"></div><button class="btn yellow" id="flashStart" type="button">👁️ ОТКРЫТЬ ФРАЗУ НА 4 СЕКУНДЫ</button><div class="vault-wordbank" id="vaultWordbank">${shuffle(words).map(w=>`<button class="vault-word" type="button">${esc(w)}</button>`).join("")}</div><div class="vault-build" id="vaultBuild">Собери фразу здесь…</div><div class="vault-input"><input id="flashInput" hidden><button class="btn primary" id="flashCheck" type="button">🔓 OPEN THE VAULT</button></div><div class="feedback" id="flashFeedback"></div><div class="vault-prize" id="vaultPrize" hidden>💎 VAULT OPEN! <span>Фраза восстановлена. Прочитай её вслух — жетон твой!</span></div></div>`;
...
function wireExit(l){
 let target=l.secret||(typeof l.story[0]==="string"?l.story[0]:l.story[0].t),d=document.getElementById("flashDisplay"),inp=document.getElementById("flashInput"),fb=document.getElementById("flashFeedback"),prize=document.getElementById("vaultPrize"),build=document.getElementById("vaultBuild"),timer=document.getElementById("vaultTimer"),chosen=[];
 document.querySelectorAll(".vault-word").forEach(b=>b.onclick=()=>{if(b.disabled)return;b.disabled=true;chosen.push(b.textContent);build.textContent=chosen.join(" ");inp.value=chosen.join(" ")});
 build.onclick=()=>{chosen=[];document.querySelectorAll(".vault-word").forEach(b=>b.disabled=false);build.textContent="Собери фразу здесь…";inp.value=""};
 document.getElementById("flashStart").onclick=()=>{d.textContent=target;chosen=[];document.querySelectorAll(".vault-word").forEach(b=>b.disabled=false);build.textContent="Собери фразу здесь…";let n=4;timer.textContent="⏳ "+n;let tick=setInterval(()=>{n--;timer.textContent=n>0?"⏳ "+n:"";if(n<=0){clearInterval(tick);d.textContent="🔒 • • • SECRET • • •";fb.textContent="Фраза исчезла! Собери её из слов 👇"}},1000)};
 document.getElementById("flashCheck").onclick=()=>{let norm=s=>s.trim().toLowerCase().replace(/[.!?,'’]/g,"").replace(/\s+/g," ");let ok=norm(inp.value)===norm(target);if(ok){d.textContent=target;fb.textContent="CLICK! 🔓 Сейф открыт!";prize.hidden=false;document.querySelector(".final-vault").classList.add("vault-open")}else{fb.textContent="Не открылся! Нажми на собранную фразу, сбрось слова и попробуй другой порядок.";prize.hidden=true}};
}

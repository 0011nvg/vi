...
function wireRead(l){
 ...
 let t=document.getElementById("toggleMarks");if(l.patterns.length){let on=true;t.onclick=()=>{on=!on;const lines=l.story.map((x,i)=>{let tx=typeof x==="string"?x:x.t,ru=typeof x==="string"?"":x.ru;return `<div class="story-line"><p>${on?highlight(tx,l.patterns):esc(tx)}</p>${ru?`<button class="story-translate" data-story-ru="${i}" type="button">🇷🇺 перевод</button><small class="story-ru" hidden>${esc(ru)}</small>`:""}</div>`}).join("");document.querySelector(".story").innerHTML=`<div class="mini-label">📖 MINI-STORY</div>`+lines;document.querySelectorAll("[data-story-ru]").forEach(b=>b.onclick=()=>{const s=b.nextElementSibling;s.hidden=!s.hidden;b.textContent=s.hidden?"🇷🇺 перевод":"Спрятать перевод"});t.textContent=on?"Спрятать код":"Показать код"}}else t.disabled=true;

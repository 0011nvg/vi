...
function gameBanner(l){return `<div class="game-banner"><span class="game-badge">🎮 GAME</span><div><b>${esc(l.practice.title.split("·")[0])}</b><small>Читай, думай, побеждай!</small></div><span class="game-skill">⭐ +1</span></div>`}</b><small>${esc(l.practice.gameNote||"Игра тренирует конкретный навык чтения.")}</small></div><span class="game-skill">READING SKILL</span></div>`}
function play(l){
